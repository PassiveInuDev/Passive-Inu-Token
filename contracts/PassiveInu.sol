...
contract TokenDividendTracker is Ownable, DividendPayingToken {
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "UDAOToken_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "UDAOToken_Dividend_Tracker: Cannot update claimWait to same value"
        );
...
contract CoinToken is ERC20, Ownable {
    ...
    address public _taxWhitelistNFT;
    ...
    constructor(
        ...
        address taxWhitelistNFT_
    ) payable ERC20(name_, symbol_) {
        _taxWhitelistNFT = taxWhitelistNFT_;
        rewardToken = addrs[0];
    ...
    function setBuyTaxes(
        ...
        require(
            rewardsFee.add(liquidity).add(marketingFee).add(deadFee) <= 25,
            "Total buy fee is over 25%"
        );
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (
                automatedMarketMakerPairs[from] &&
                IERC721(_taxWhitelistNFT).balanceOf(to) == 0
            ) {
